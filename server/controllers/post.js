...
export const createPost = async (req, res) => {
  ...
  if (!content.length) return res.json({ error: "write something" });
  try {
    const post = new Post({ content, image, postedBy: req.auth._id });
    //to add some fields in the response, we need to search the new post in the db, then use populate method
    await post.save();
...
export const getPost = async (req, res) => {
  try {
    const post = await Post.findById(req.params._id)
      .populate("postedBy", "_id image name")
      .populate("comments.postedBy", "_id image name");
    console.log("hi single post", post);
    res.json(post);
  } catch (error) {
    console.log(error);
  }
